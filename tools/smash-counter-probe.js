...
// 真人控制; crouch=false 对照站立
function humanControl(engine, crouch) {
  ...
  // 提前起拍(同 AI preSwing): 球将在命中窗(0.08~0.28s)内进箱即按推球,
  // 不等本方弹台——快球(15m/s+)弹台后仅剩 0.05s, 等弹台再起拍必漏
  if (incoming) {
...
function trial(aimX, hitY, crouch) {
...
function run(label, crouch, n) {
  ...
  while (t < n && attempts < 2000) {
    ...
    const d = trial(aimX, hitY, crouch);
...
run('蹲下+推球 反击引擎扣杀', true, 250);
run('站立推球 反击引擎扣杀(对照)', false, 250);
