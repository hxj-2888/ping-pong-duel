      ...
      if (incoming && smashIn && !inBox && cross && cross.t < 0.18) {
        ...
        if (s.catchOk && (!smashIn || s.defOk)) {
          ...
          if (s.hitDelay >= react) {
            ...
            // 概率 = 等级基准 × 数值因子(四个微调倍率均值) + 未变招回合累积(每回合 +0.10)，封顶 100%，变招后清空
            if (!s.trickRolled) {
              s.trickRolled = true;
              s.exch++;
              if (s.exch >= 2) {
                const numF = (t.reactMul == null ? 1 : t.reactMul) + (t.catchMul == null ? 1 : t.catchMul) +
                  (t.smashMul == null ? 1 : t.smashMul) + (t.agilityMul == null ? 1 : t.agilityMul);
                if (rnd(s) < Math.min(1, (L.trickBase || 0) * numF / 4 + s.trickAccum)) {
                  s.trickOn = true;
