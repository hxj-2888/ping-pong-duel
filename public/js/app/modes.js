...
(function () {
  ...
  function startOnlineGame(side) {
    ...
    if (PPD.isTouch) {
      PPD.ui.hintBar.innerHTML =
        '左下摇杆全方位移动 · 右下<b>蹲</b>按钮（蹲下可接贴地球，蹲得越久越慢；3秒内反复蹲站转换延迟增至最多0.5秒）· 发球：<b>点一下屏幕开始瞄准，移动手指调整轨迹，再点一下发球</b> · 对打：单击推球 / 双击扣球';
    }
    PPD.showTouch(true);
  }
  ...
  function startLocal() {
    ...
    if (PPD.isTouch) {
      PPD.ui.hintBar.innerHTML = '左下摇杆全方位移动 · 右下<b>蹲</b>按钮（蹲得越久越慢；3秒内反复蹲站转换延迟增至最多0.5秒）· 发球：点一下开始瞄准，移动手指调整轨迹，再点一下发球（左半屏 P1，右半屏 P2）· 对打：单击推球 / 双击扣球';
    }
  ...
  function startAI() {
    ...
    if (PPD.isTouch) {
      PPD.ui.hintBar.innerHTML =
        `左下摇杆全方位移动 · 右下<b>蹲</b>按钮（蹲得越久越慢；3秒内反复蹲站转换延迟增至最多0.5秒）· 发球：<b>点一下开始瞄准，移动手指调整轨迹，再点一下发球</b> · 对打：单击推球 / 双击扣球 · 电脑难度：${L.name}`;
    }
    PPD.GameAudio.ensure();
    PPD.showTouch(true);
  }
