...
// --- 2b. 低平快球来球：球在箱内 → 挥拍第一帧即命中（免起拍延迟，与扣杀反击一致） ---
{
  const { e } = make(3);
  const t = stepCount(e, 20);
  check('低平快球来球：挥拍第一帧判箱即命中（t=1/120，免 0.08s 起拍延迟）', t === 2);
  check('低平快球来球：命中方为本方（球已被接回）', e.ball.hitBy === 0);
}
